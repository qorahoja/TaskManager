...
data = {
    'task_name': 'Jdjsjd',
    'task_description': 'Hshsjjs',
    'selected_members': [{'member_id': 'Abu Bakr'}, {'member_id': 'Abu Bakr'}]
}

# Extract unique member_ids
unique_members = list({member['member_id'] for member in data['selected_members']})

print(unique_members)  # Output: ['Abu Bakr']
